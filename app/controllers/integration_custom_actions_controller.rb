class IntegrationCustomActionsController < ApplicationController
  ...
  def action_missing(action)
    # capture and redirect all actions to internal handlers
  end
  ...
  def set_account_id
    ...
    @account_id = params[:account_id] || session[:integration_session][:account_id]
    session[:integration_session] = session[:integration_session].merge(:account_id => @account_id)
  end
